...
const blogContentInput = document.querySelector("#postInputField");
const postBlogBtn = document.querySelector(".postBlogBtn");
const blogPostArea = document.querySelector(".blogPostArea");
...
async function getUserData(userUid) {
    try {
        ...
        if (docSnap.exists()) {
            const { userFirstName, userSurName } = docSnap.data();
            userFullName.textContent = `${userFirstName} ${userSurName}`;
        } else {
            console.log("No such document!");
        }
...
// Enable/disable post button based on input fields
const enablePostBtn = () => {
    postBlogBtn.disabled = !(blogTitleInput.value && blogContentInput.value);
};
...
const postBlogHandler = async () => {
    try {
        ...
        if (blogTitle.length < 5 || blogTitle.length > 50) {
            // Handle invalid blog title length
            return;
        }

        if (blogContent.length < 100 || blogContent.length > 3000) {
            // Handle invalid blog content length
            return;
        }
...
// Event listeners
postBlogBtn.addEventListener("click", postBlogHandler);
blogTitleInput.addEventListener("input", enablePostBtn);
blogContentInput.addEventListener("input", enablePostBtn);
